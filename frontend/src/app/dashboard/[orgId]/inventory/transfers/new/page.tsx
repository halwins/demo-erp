...
import { ArrowLeft, Plus, Trash2, Save, ArrowRightLeft } from 'lucide-react';
...
export default function NewTransferPage({ 
  ...
  const [sourceWarehouseId, setSourceWarehouseId] = useState<string>(defaultWarehouseId || '');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Internal transfers create an OUT document at the source, and an IN at the dest automatically.
      const doc = await createInventoryDocument(orgId, sourceWarehouseId, {
        documentType: 'TRANSFER_OUT',
        transferSourceWarehouseId: destWarehouseId, // destination
        scheduledDate: new Date(scheduledDate).toISOString(),
        notes,
        items
      });
      toast.success('Internal transfer document created successfully');
      router.push(`${APP_ROUTES.INVENTORY.DOCUMENT_DETAIL(orgId, doc.id)}?whId=${sourceWarehouseId}`);
    } catch (err) {
      console.error(err);
      toast.error('Failed to create transfer document');
    } finally {
      ...
      <div className="px-6 py-4 border-b border-[#e0e0e0] flex items-center justify-between shrink-0">
        <div className="flex items-center space-x-3">
          ...
          <div className="flex items-center space-x-2 text-[#242424]">
            <ArrowRightLeft className="w-5 h-5 text-[#8e44ad]" />
            <h2 className="text-[18px] font-[600]">New Internal Transfer</h2>
          </div>
        </div>
        ...
        <div className="max-w-[1000px] mx-auto space-y-6">
          ...
          <div className="bg-white border border-[#e0e0e0] rounded-[6px] shadow-sm p-6">
            <div className="flex justify-between items-center mb-4 border-b border-[#f0f0f0] pb-2">
              <h3 className="text-[14px] font-[700] text-[#242424]">Products to Transfer</h3>
              <Button 
